...
int SyncDocs()
{
    ...
    // Compile guard for every README.md csharp fence (see RenderReadmeExamples).
    var readmeGuard = RenderReadmeExamples(File.ReadAllText("README.md"));
    changed |= WriteIfChanged("tests/RoxyApi.Tests/Generated/ReadmeExamples.g.cs", readmeGuard);
...
// ─── README example rendering ────────────────────────────────────────────────
//
// Every README.md csharp fence compiles as its own method, so an example that stops compiling fails the build.
// README examples read as one narrative, so ReadmeScaffold supplies the declarations a later fence reuses, each
// copied from where the README first shows it; the caller-supplied auth provider and HttpClient get typed stubs.
// A scaffold line is added on any whole-word use; a spurious unused local is silenced by the file pragma.
//
// Render every README.md ```csharp fence as its own compiled method. Mirrors RenderMethods
// in spirit (spec-derived text only, never hand-restated), but this source is prose, not
// JSON, so the scaffold and regexes below are locals of this function rather than a
// top-level declaration: this file's entry point returns before reaching a plain statement
// placed after it, so anything the renderer needs must be initialized when it actually runs.
string RenderReadmeExamples(string readme)
{
    (string Name, string Decl, string? ExtraUsing)[] scaffold =
    [
        ("roxy", "var roxy = new RoxyClient(Environment.GetEnvironmentVariable(\"ROXY_API_KEY\")!);", null),
        ("place", "var place = await roxy.Location.Search.GetAsync(c => c.QueryParameters.Q = \"New York\");", null),
        ("city", "var city = place!.Cities![0];", null),
        ("latitude", "var latitude = city.Latitude;", null),
        ("longitude", "var longitude = city.Longitude;", null),
        ("timezone", "var timezone = city.Timezone;", null),
        ("birthDate", "var birthDate = new Date(1990, 1, 15);", null),
        ("birthTime", "var birthTime = new Time(14, 30, 0);", null),
        ("london", "var london = await roxy.Location.Search.GetAsync(c => c.QueryParameters.Q = \"London\");", null),
        ("partnerCity", "var partnerCity = london!.Cities![0];", null),
        ("lat2", "var lat2 = partnerCity.Latitude;", null),
        ("lon2", "var lon2 = partnerCity.Longitude;", null),
        ("tz2", "var tz2 = partnerCity.Timezone;", null),
        ("partnerDate", "var partnerDate = new Date(1992, 7, 22);", null),
        ("partnerTime", "var partnerTime = new Time(9, 0, 0);", null),
        ("authProvider", "IAuthenticationProvider authProvider = null!;", "Microsoft.Kiota.Abstractions.Authentication"),
        ("yourHttpClient", "var yourHttpClient = new HttpClient();", null),
    ];

    var fenceRegex = new Regex(@"```csharp\r?\n(.*?)```", RegexOptions.Singleline);
    var headingRegex = new Regex(@"^#{1,6}\s+(.*)$", RegexOptions.Multiline);
    var usingMentionRegex = new Regex(@"\busing\s+([A-Za-z_][\w.]*)\s*;");
    var usingStatementLineRegex = new Regex(@"^using\s+[A-Za-z_][\w.]*\s*;\s*(//.*)?$");

    var headings = headingRegex.Matches(readme).Select(m => (Index: m.Index, Heading: m.Groups[1].Value.Trim())).ToList();
    string HeadingBefore(int pos)
    {
        var current = "Intro";
    ...
    var sb = new StringBuilder();
    sb.AppendLine("// <auto-generated/> Compile guard for the README.md csharp examples.");
    sb.AppendLine("// Regenerated by `dotnet run --project tools/RoxyDevTools -- sync-docs`. Do not edit by hand.");
    sb.AppendLine("#pragma warning disable");
    sb.AppendLine("using Microsoft.Kiota.Abstractions;");
    sb.AppendLine("using RoxyApi;");
    sb.AppendLine("using RoxyApi.Models;");
    sb.AppendLine();

    var index = 0;
    foreach (Match fence in fenceRegex.Matches(readme))
    {
        index++;
        var heading = HeadingBefore(fence.Index);
        var raw = fence.Groups[1].Value;

        // Hoist every `using X;` mention (a real leading statement, or one named only in a
        // comment, as FengShui and Vastu do to keep the fence terse) into this block's own
        // namespace-scoped usings; a bare `using` directive is not valid inside a method body.
        var mentionedUsings = usingMentionRegex.Matches(raw).Select(m => m.Groups[1].Value).ToHashSet();
        var bodyLines = raw.Replace("\r\n", "\n").Split('\n').Where(l => !usingStatementLineRegex.IsMatch(l.Trim())).ToList();
        while (bodyLines.Count > 0 && bodyLines[0].Trim().Length == 0) bodyLines.RemoveAt(0);
        while (bodyLines.Count > 0 && bodyLines[^1].Trim().Length == 0) bodyLines.RemoveAt(bodyLines.Count - 1);
        var body = string.Join('\n', bodyLines);

        // A scaffold declaration can itself reference an earlier one (latitude reads city,
        // city reads place, place reads roxy), so pulling one in can pull in more: close over
        // that until nothing new is needed. Self-declared names are never pulled in, and the
        // fixed point always terminates since a name is only ever added once.
        var declaresItself = new Func<string, bool>(name => Regex.IsMatch(body, $@"\bvar\s+{Regex.Escape(name)}\s*="));
        var isReferenced = new Func<string, bool>(name => Regex.IsMatch(body, $@"\b{Regex.Escape(name)}\b"));
        var includedNames = new HashSet<string>();
        bool grew;
        do
        {
            grew = false;
            foreach (var s in scaffold)
            {
                if (includedNames.Contains(s.Name) || declaresItself(s.Name)) continue;
                var neededByBody = isReferenced(s.Name);
                var neededByAnIncludedDecl = scaffold.Any(inc => includedNames.Contains(inc.Name) && Regex.IsMatch(inc.Decl, $@"\b{Regex.Escape(s.Name)}\b"));
                if (neededByBody || neededByAnIncludedDecl) { includedNames.Add(s.Name); grew = true; }
            }
        } while (grew);
        var preamble = scaffold.Where(s => includedNames.Contains(s.Name)).ToList();

        var extraUsings = mentionedUsings
            .Concat(preamble.Where(s => s.ExtraUsing is not null).Select(s => s.ExtraUsing!))
            .Where(u => u is not ("RoxyApi" or "RoxyApi.Models" or "Microsoft.Kiota.Abstractions"))
            .Distinct().OrderBy(u => u, StringComparer.Ordinal).ToList();

        sb.AppendLine("namespace RoxyApi.Tests.Generated.ReadmeExamples");
        sb.AppendLine("{");
        foreach (var u in extraUsings) sb.AppendLine($"    using {u};");
        if (extraUsings.Count > 0) sb.AppendLine();
        sb.AppendLine($"    // README.md \"{heading}\"");
        sb.AppendLine($"    internal static class ReadmeExample_{index:D2}_{ReadmeSlug(heading)}");
        sb.AppendLine("    {");
        sb.AppendLine("        public static async Task Run()");
        sb.AppendLine("        {");
        foreach (var line in preamble) sb.AppendLine("            " + line.Decl);
        if (preamble.Count > 0) sb.AppendLine();
        foreach (var line in bodyLines) sb.AppendLine(line.Length == 0 ? "" : "            " + line);
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine("}");
        sb.AppendLine();
...
// A README heading as a PascalCase identifier: "6. Feng shui API (Kua number, ...)" ->
// "FengShuiApi". Strips a leading catalog number and any parenthetical qualifier, since
// neither carries meaning once the fence has its own numeric, always-unique prefix.
string ReadmeSlug(string heading)
{
    var cleaned = Regex.Replace(heading, @"^\d+\.\s*", "");
    cleaned = Regex.Replace(cleaned, @"\(.*?\)", "");
    var words = Regex.Matches(cleaned, @"[A-Za-z0-9]+").Select(w => char.IsDigit(w.Value[0]) ? "N" + w.Value : char.ToUpperInvariant(w.Value[0]) + w.Value[1..]);
    var slug = string.Concat(words);
    return slug.Length == 0 ? "Block" : slug;
}
